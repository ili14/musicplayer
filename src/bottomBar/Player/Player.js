...
const Player = React.memo(
	({ playing, musicSrc, onSeek, onEnded, onPlay = () => {} }) => {
		...
		const handleSeekBarMouseMove = e => {
			...
			seekBarPointerRef.current.style.marginLeft = left + "px";
		};
		...
		const handleSeekBarTouchMove = e => {
			const seekBarRect = e.target.getBoundingClientRect();
			const seekBarWidth = seekBarRect.right - seekBarRect.left;
			let newSeekBarPointerMarginLeft = e.touches[0].clientX - seekBarRect.left;
			if (
		...
		React.useEffect(() => {
			...
			} else audioRef.current.pause(); console.log("stop music");
		}, [playing]);
		...
		return (
			<div className={styles.container}>
				<div
					...
					ref={seekBarRef}
				>
					<div className={styles.pointer} ref={seekBarPointerRef}></div>
					<div
